...
void    lexical_errors(t_errors error, char *line, t_counter *counter)
{
	...
	*/
	...
	else if (error == NO_END_QUOTES)
	{
		ft_printf("\x1b[31mLexical error\x1b[0m[%zu:%zu]: The name's of string doesn't have END quotes!\n", counter->row, counter->column);
		point_to_mistake(line, counter->column, STRING_QUOTES_CHAR);
	}
	else if (error == UNMATCHED_COMMAND)
	{
		ft_printf("\x1b[31mLexical_error\x1b[0m[%zu:%zu]: Unmatched command!\n", counter->row, counter->column);
		point_to_mistake(line, counter->column, ENDSTRING_CHAR);
	}
	else if (error == CHAMPION_NAME_TOO_LONG)
	{
		ft_printf("\x1b[31mLexical_error\x1b[0m[%zu:%zu]: CHAMPION_NAME_TOO_LONG!\n", counter->row, counter->column);
		point_to_mistake(line, counter->column, ENDSTRING_CHAR);
	}
	else if (error == CHAMPION_COMMENT_TOO_LONG)
	{
		ft_printf("\x1b[31mLexical_error\x1b[0m[%zu:%zu]: CHAMPION_COMMENT_TOO_LONG!\n", counter->row, counter->column);
		point_to_mistake(line, counter->column, ENDSTRING_CHAR);
	}
	exit(EXIT_FAILURE);
}
...
void    syntactic_errors(t_errors error, char *line, t_counter *counter)
{
	...
	**
	*/
	if (error == NOT_ALL_COMMAND)
	{
		ft_printf("Syntactic error[%zu:%zu]: Is not a .name or .comment!\n", counter->row, counter->column);
		point_to_mistake(line, counter->column, ENDSTRING_CHAR);
	}
	exit(EXIT_FAILURE);
}
...
void    semantic_errors(t_errors error, char *line, t_counter *counter)
{
	...
	**
	*/
	if (error == DOUBLE_NAME)
		ft_printf("Semantic error[%zu:%zu]: The name command has already read!\n", counter->row, counter->column);
	else if (error == DOUBLE_COMMENT)
	{
